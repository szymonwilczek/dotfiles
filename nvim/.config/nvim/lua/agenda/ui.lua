...
local function build_right_side(monday_str, today_str, projects_db, week_data)
  ...
  -- Title & Header (Pn, Wt, Śr, Cz, Pt, So, Nd)
  ...
  add_r_line { { text = '📌 DZISIEJSZY FOCUS:', hl = 'AgendaHeader' } }
  ...
  add_r_line { { text = '  ' .. focus_proj_str, hl = #today_data.projects > 0 and 'AgendaTodo' or 'AgendaColorGray' } }
  ...
  if #today_data.tasks > 0 then
    ...
    add_r_line { { text = string.format('  Zadania: %d do zrobienia', active), hl = 'AgendaColorOrange' } }
  end
  ...
  for offset = 1, 2 do
    ...
    add_r_line { { text = string.format('- %s: %s', u_polish, u_proj_str), hl = 'AgendaColorGray' } }
  end
...
function M.render(monday_str)
  ...
  local header_text =
    string.format('📅  PLANNER - TYDZIEŃ %s (%s - %s)', os.date('%V', start_time), os.date('%d.%m', start_time), os.date('%d.%m', end_time))
  ...
  for _, day in ipairs(day_keys) do
    ...
    -- Format day name and pad to 12 chars to align dashes perfectly regardless of locale
    ...
    -- Added an explicit space before the dash ' - ' to align dashes beautifully and avoid hugging Poniedziałek
    local left_segs = {}
    ...
    if #day_data.projects == 0 then
      ...
      for idx, p_name in ipairs(day_data.projects) do
        ...
        local arrow = ''
        if proj.type == 'serwis' then arrow = is_collapsed and ' ' or ' ' end
    ...
    if total_tasks > 0 then
      ...
      -- Insert spacing to push progress bar to the end of left column
...
function M.open(monday_str)
  ...
  -- Folding disabled (using custom folds instead of Vim folds)
  vim.wo.foldmethod = 'manual'
  vim.wo.foldcolumn = '0'

  -- Window Resize Autocmd for responsiveness
  ...
  -- Initial highlight configuration called once
  setup_highlights()
